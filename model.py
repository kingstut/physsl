...
class CoherenceNet(nn.Module):
    def __init__(self, arch="resnet50", wave_coeff=10**(-6), diff_coeff=10**(-2), coh_coeff=1):
        super().__init__()
        ...
        self.wave_coeff, self.diff_coeff, self.coh_coeff = wave_coeff, diff_coeff, coh_coeff
    ...
    def forward(self, x, y):
        ...
        f, coh_x_y = coherence(wave_x.detach().numpy(), wave_y.detach().numpy())
        
        loss_coh = torch.mean((1 - torch.from_numpy(coh_x_y))**2)
        ...
        print(loss_wave, loss_diff, loss_coh)
        return loss
